...
contract ChainflipToUniswapV3Connector {
    ...
    function swap(address tokenIn, address tokenOut, uint256 amountIn, bool simulate) public returns (uint256 amountOut, uint256 gasUsed) {
        ...
        (,amountOut) = _swapWithPool(bestPool, tokenIn, tokenOut, amountIn, simulate);

        gasUsed = startingGas - gasleft();
    }

    function findOptimalPoolForSwap(address tokenIn, address tokenOut) public view returns (address[] memory pools, uint256[] memory prices, address poolWithBestSpotPrice) {
    ...
    function uniswapV3SwapCallback(int256 amount0Delta, int256 amount1Delta, bytes memory path) external {
    ...
    function _swapWithPool(address pool, address tokenIn, address tokenOut, uint256 amountIn, bool simulate) private returns (uint256 actualAmountIn, uint256 actualAmountOut) {
    ...
    function _raise(bytes memory data) private pure {
        assembly {
            let size := mload(data)
